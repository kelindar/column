...
func (c *columnFloat32) Apply(r *commit.Reader) {
	for r.Next() {
		...
		switch r.Type {
		case commit.Put:
			c.data[r.Offset] = float32(r.AsFloat32())

		// If this is an atomic increment/decrement, we need to change the operation to
		// the final value, since after this update an index needs to be recalculated.
		case commit.Add:
			value := c.data[r.Offset] + float32(r.AsFloat32())
...
func (c *columnFloat64) Apply(r *commit.Reader) {
	for r.Next() {
		...
		switch r.Type {
		case commit.Put:
			c.data[r.Offset] = float64(r.AsFloat64())

		// If this is an atomic increment/decrement, we need to change the operation to
		// the final value, since after this update an index needs to be recalculated.
		case commit.Add:
			value := c.data[r.Offset] + float64(r.AsFloat64())
			c.data[r.Offset] = value
			r.SwapFloat64(value)
		}
	}
...
func (c *columnInt) Apply(r *commit.Reader) {
	for r.Next() {
		...
		switch r.Type {
		case commit.Put:
			c.data[r.Offset] = int(r.AsInt())

		// If this is an atomic increment/decrement, we need to change the operation to
		// the final value, since after this update an index needs to be recalculated.
		case commit.Add:
			value := c.data[r.Offset] + int(r.AsInt())
			c.data[r.Offset] = value
			r.SwapInt(value)
		}
	}
...
func (c *columnInt16) Apply(r *commit.Reader) {
	for r.Next() {
		...
		switch r.Type {
		case commit.Put:
			c.data[r.Offset] = int16(r.AsInt16())

		// If this is an atomic increment/decrement, we need to change the operation to
		// the final value, since after this update an index needs to be recalculated.
		case commit.Add:
			value := c.data[r.Offset] + int16(r.AsInt16())
			c.data[r.Offset] = value
			r.SwapInt16(value)
		}
	}
...
func (c *columnInt32) Apply(r *commit.Reader) {
	for r.Next() {
		...
		switch r.Type {
		case commit.Put:
			c.data[r.Offset] = int32(r.AsInt32())

		// If this is an atomic increment/decrement, we need to change the operation to
		// the final value, since after this update an index needs to be recalculated.
		case commit.Add:
			value := c.data[r.Offset] + int32(r.AsInt32())
			c.data[r.Offset] = value
			r.SwapInt32(value)
		}
	}
...
func (c *columnInt64) Apply(r *commit.Reader) {
	for r.Next() {
		...
		switch r.Type {
		case commit.Put:
			c.data[r.Offset] = int64(r.AsInt64())

		// If this is an atomic increment/decrement, we need to change the operation to
		// the final value, since after this update an index needs to be recalculated.
		case commit.Add:
			value := c.data[r.Offset] + int64(r.AsInt64())
			c.data[r.Offset] = value
			r.SwapInt64(value)
		}
	}
...
func (c *columnUint) Apply(r *commit.Reader) {
	for r.Next() {
		...
		switch r.Type {
		case commit.Put:
			c.data[r.Offset] = uint(r.AsUint())

		// If this is an atomic increment/decrement, we need to change the operation to
		// the final value, since after this update an index needs to be recalculated.
		case commit.Add:
			value := c.data[r.Offset] + uint(r.AsUint())
			c.data[r.Offset] = value
			r.SwapUint(value)
		}
	}
...
func (c *columnUint16) Apply(r *commit.Reader) {
	for r.Next() {
		...
		switch r.Type {
		case commit.Put:
			c.data[r.Offset] = uint16(r.AsUint16())

		// If this is an atomic increment/decrement, we need to change the operation to
		// the final value, since after this update an index needs to be recalculated.
		case commit.Add:
			value := c.data[r.Offset] + uint16(r.AsUint16())
			c.data[r.Offset] = value
			r.SwapUint16(value)
		}
	}
...
func (c *columnUint32) Apply(r *commit.Reader) {
	for r.Next() {
		...
		switch r.Type {
		case commit.Put:
			c.data[r.Offset] = uint32(r.AsUint32())

		// If this is an atomic increment/decrement, we need to change the operation to
		// the final value, since after this update an index needs to be recalculated.
		case commit.Add:
			value := c.data[r.Offset] + uint32(r.AsUint32())
			c.data[r.Offset] = value
			r.SwapUint32(value)
		}
	}
...
func (c *columnUint64) Apply(r *commit.Reader) {
	for r.Next() {
		...
		switch r.Type {
		case commit.Put:
			c.data[r.Offset] = uint64(r.AsUint64())

		// If this is an atomic increment/decrement, we need to change the operation to
		// the final value, since after this update an index needs to be recalculated.
		case commit.Add:
			value := c.data[r.Offset] + uint64(r.AsUint64())
			c.data[r.Offset] = value
			r.SwapUint64(value)
		}
	}
